...
MESSAGES: dict[str, str] = {
    ...
    "deploy.disk_low": "{host}에 {free}GB만 남았습니다. {need}GB가 필요합니다. 정리 방법: docker image prune -af && docker builder prune -af",
    "deploy.pruning": "Docker 디스크 공간 회수 중(미사용 이미지 및 오래된 빌드 캐시)...",
    "deploy.prune_failed": "Docker 디스크 공간을 회수하지 못했습니다. 배포 자체는 성공했습니다.",
    ...
    "plugins.no_external_installed": "설치된 외부 플러그인이 없습니다. 다음으로 설치할 수 있습니다: pip install aegis-stack-<name>",
    ...
    "plugins.update_not_pip_installed": "플러그인 '{name}'은(는) 프로젝트의 _plugins 목록에 있지만 pip로 설치되어 있지 않습니다. 먼저 `pip install {dist}`을 실행하세요.",
    ...
    "plugins.help_create": "새로운 aegis-stack-<name> Python 패키지의 스캐폴드를 생성합니다.",
    "plugins.help_arg_create_name": "플러그인 이름 (소문자, 하이픈 없음). Python 패키지 이름 aegis_stack_<name> 및 설치 이름 aegis-stack-<name>으로 사용됩니다.",
    ...
    "plugins.create_next_steps_edit_comment": "src/aegis_stack_<name>/plugin.py를 편집해 와이어링 추가",
    ...
    "plugins.search_install_hint": "현재 방식: pip install aegis-stack-<name>을 실행한 다음 aegis plugins list.",
    ...
    "build.step.revisions": "모델에서 리비전 생성 중",
