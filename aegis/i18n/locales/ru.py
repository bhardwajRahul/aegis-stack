...
MESSAGES: dict[str, str] = {
    ...
    "deploy.disk_low": "На {host} свободно всего {free} ГБ, требуется {need} ГБ. Освободить: docker image prune -af && docker builder prune -af",
    "deploy.pruning": "Освобождение места Docker (висячие образы и старый кэш сборки)...",
    "deploy.prune_failed": "Не удалось освободить место Docker; сам деплой прошёл успешно.",
    ...
    "plugins.no_external_installed": "Внешние плагины не установлены. Установить можно через: pip install aegis-stack-<name>",
    ...
    "plugins.update_not_pip_installed": "Плагин «{name}» присутствует в _plugins проекта, но не установлен через pip; сначала выполните `pip install {dist}`.",
    ...
    "plugins.help_create": "Сгенерировать каркас нового Python-пакета aegis-stack-<name>.",
    "plugins.help_arg_create_name": "Имя плагина (нижний регистр, без дефисов). Используется как имя Python-пакета aegis_stack_<name> и имя установки aegis-stack-<name>.",
    ...
    "plugins.create_next_steps_edit_comment": "отредактировать src/aegis_stack_<name>/plugin.py и добавить связку",
    ...
    "plugins.search_install_hint": "Пока что: pip install aegis-stack-<name>, затем aegis plugins list.",
