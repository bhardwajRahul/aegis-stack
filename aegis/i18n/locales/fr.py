...
MESSAGES: dict[str, str] = {
    ...
    "deploy.disk_low": "Seulement {free} Go libres sur {host}, il en faut {need}. Libérez de l'espace avec : docker image prune -af && docker builder prune -af",
    "deploy.pruning": "Libération de l'espace disque Docker (images orphelines et ancien cache de build)...",
    "deploy.prune_failed": "Impossible de libérer l'espace disque Docker ; le déploiement lui-même a réussi.",
    ...
    "plugins.no_external_installed": "Aucun plugin externe installé. Installez-en un avec : pip install aegis-stack-<name>",
    ...
    "plugins.update_not_pip_installed": "Le plugin « {name} » figure dans _plugins du projet mais n'est pas installé via pip ; lancez d'abord `pip install {dist}`.",
    ...
    "plugins.help_create": "Générer le squelette d'un nouveau paquet Python aegis-stack-<name>.",
    "plugins.help_arg_create_name": "Nom du plugin (minuscules, sans tirets). Devient le paquet Python aegis_stack_<name> et le nom d'installation aegis-stack-<name>.",
    ...
    "plugins.create_next_steps_edit_comment": "modifiez src/aegis_stack_<name>/plugin.py pour ajouter le câblage",
    ...
    "plugins.search_install_hint": "Pour l'instant : pip install aegis-stack-<name>, puis aegis plugins list.",
