...
MESSAGES: dict[str, str] = {
    ...
    "deploy.disk_low": "{host} の空き容量が {free}GB しかありません（{need}GB 必要）。回収方法：docker image prune -af && docker builder prune -af",
    "deploy.pruning": "Docker のディスク領域を回収中（未使用イメージと古いビルドキャッシュ）...",
    "deploy.prune_failed": "Docker のディスク領域を回収できませんでした。デプロイ自体は成功しています。",
    ...
    "plugins.no_external_installed": "外部プラグインはインストールされていません。pip install aegis-stack-<name> でインストールできます。",
    ...
    "plugins.update_not_pip_installed": "プラグイン「{name}」はプロジェクトの _plugins リストに含まれていますが、pip でインストールされていません。先に `pip install {dist}` を実行してください。",
    ...
    "plugins.help_create": "新しい aegis-stack-<name> Python パッケージのひな型を作成します。",
    "plugins.help_arg_create_name": "プラグイン名（小文字、ハイフンなし）。Python パッケージ名 aegis_stack_<name> およびインストール名 aegis-stack-<name> として使用されます。",
    ...
    "plugins.create_next_steps_edit_comment": "src/aegis_stack_<name>/plugin.py を編集して配線を追加",
    ...
    "plugins.search_install_hint": "現時点では：pip install aegis-stack-<name> を実行してから aegis plugins list。",
