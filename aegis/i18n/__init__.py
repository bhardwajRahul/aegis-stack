...
Usage:
    ...
    app = typer.Typer(help=lazy_t("plugins.help"))
    name: str = typer.Argument(..., help=lazy_t("init.help_arg_name"))
...
def lazy_t(key: str, **kwargs: object) -> str:
    """Lazy translation for typer help= parameters.

    Returns a ``_Lazy`` object that resolves to the translated string
    when click/typer accesses it for display. The return type is cast
    to ``str`` so it satisfies typer's ``help: Optional[str]`` signature
    while still deferring resolution until display time.
    ...
    Returns:
        Lazy translation object (typed as str for typer compatibility)
    """
    return cast(str, _Lazy(key, **kwargs))
